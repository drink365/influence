...
df_assets = pd.DataFrame([{"項目": k, "金額（萬元）": float(v)} for k, v in asset_items.items()]).sort_values("金額（萬元）", ascending=False)
df_liab = pd.DataFrame([{"項目": k, "金額（萬元）": float(v)} for k, v in liab_items.items()]).sort_values("金額（萬元）", ascending=False)
...
with col_left:
    ...
    if df_assets_nonzero.empty:
        ...
        fig = px.pie(df_assets_nonzero, names="項目", values="金額（萬元）", hole=0.35, title="資產配置比例（單位：萬元）")
...
with col_right:
    ...
    with st.expander("資產明細（萬元）", expanded=True):
        ...
        st.download_button("下載資產 CSV（萬元）", data=df_to_csv_bytes(df_assets), file_name="資產明細_萬元.csv", mime="text/csv", use_container_width=True)
    with st.expander("負債明細（萬元）", expanded=True):
        st.dataframe(df_liab, use_container_width=True)
        st.download_button("下載負債 CSV（萬元）", data=df_to_csv_bytes(df_liab), file_name="負債明細_萬元.csv", mime="text/csv", use_container_width=True)
...
with cA:
    st.download_button("下載 PDF 摘要（萬元）", data=build_pdf_bytes(), file_name="資產地圖_摘要_萬元.pdf", mime="application/pdf", use_container_width=True)
with cB:
    df_all = pd.concat([df_assets.assign(類別="資產"), df_liab.assign(類別="負債")], ignore_index=True)
    st.download_button("下載總表 CSV（萬元）", data=df_to_csv_bytes(df_all[["類別", "項目", "金額（萬元）"]]), file_name="資產地圖_總表_萬元.csv", mime="text/csv", use_container_width=True)
